...
Agent Playbook 是 [照见 Pulsar](https://github.com/sou350121/Pulsar-KenVersion) 在 AI 应用生态领域的知识输出端。

Pulsar 在后台持续做的事：

- **全自动运行** — 33 个 cron job，每天 07:00 北京时间开始，无需人工触发
- **自愈 Watchdog** — 15 项健康检查，RSS 中断 · LLM 超时 · 社交情报断档，故障自动恢复
- **专家评分兜底** — 每条输出经 qwen3.5-plus 专家评分，低于阈值直接丢弃，不推送
- **月度假设校准** — 维护 19 条 AI 应用生态假设，每月自动更新置信度，系统越用越准
- **双周预测验证** — 每两周报告含可验证预测，下一期必须打分（✅/❌/⏳），正确率有历史记录
